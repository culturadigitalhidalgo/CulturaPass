...
//plantilla V4.0
    ...
    $tablabase="municipios";
    $idbase="idmunicipio";
    $paginabase=$selfpag;
    
    //Variables Globales del Archivo (pueden ser omitidas) texto desplegado en los formularios de catálogos
    $textoencabezado="Municipios";
    $textoencabezadoforms="Municipio";
    
    //Variables para obtención de información en grid data table
    $camposrs="estados.idestado as IDEstado, estado as Estado, idmunicipio as IDMunicipio, municipio as Municipio";
    $tablars=$tablabase." join estados on municipios.idestado= estados.idestado";
    $wherers="1";
    $idaccion="IDMunicipio";
    ...
    if(isset($_POST['delete'])){
        ...
        if(($delete = $consultas->to_delete_trans($tablabase,$idbase."='".$_POST['delete']."' and idestado='".$_POST['idestado']."'"))===false){
            $valid=false;
            echo 'Error al procesar la información<br>';
        }
    ...
    if(isset($_POST['update'])){
        ...
        //Guardado de información en la tabla declarada en la variable global $tablabase y la información en forma de Array $data 
        if(($update = $consultas->to_update_trans($tablabase, $data, $idbase."='".$_POST['update']."' and idestado='".$_POST['idestadoo']."'"))===false){
            $valid=false;
            echo 'Error al procesar la información<br>';
        }
        ...
        <script>
            $(document).ready(function(){
                ...
                $('#informacion').on('click','a.modificar',function(e){
                    ...
                    infoData.append('where','<?php echo $idbase;?>='+id+" and idestado="+idestado);
                ...
                $('body').on('change','select',function(){
                    ...
                    switch(id){
                        case 'idestado':
                            var formData = new FormData();
                            formData.append('obtenerselect',"idjurisdiccion,concat(idjurisdiccion,' ',jurisdiccion) as jurisdiccion");
                            formData.append('tabla','jurisdicciones');
                            formData.append('where',"idestado='"+value+"'");
                            //procesar información
                            
                            $('#idjurisdiccion').html('<option value="">Seleccione...</option>'+obtener_info(formData,false));
                        ...
                        case 'idestadom':
                            ...
                            $('#idjurisdiccionm').html('<option value="">Seleccione...</option>'+obtener_info(formData,false));
                                ...
                                while ($row = mysql_fetch_array($data)){
                                    echo '<tr>';
                                        ...
                                        echo '<td style="width:50px; min-width:50px; text-align:right;">
                                            <a id-accion="'.$row[$idaccion].'" idestado="'.$row['IDEstado'].'" class="modificar"><span class="fa fa-pencil-square-o" title="Modificar"></span></a> 
                                            <a id-accion="'.$row[$idaccion].'" idestado="'.$row['IDEstado'].'" class="eliminar"><span class="fa fa-trash-o" title="Eliminar"></span></a>
                                        </td>
                                    </tr>';
                                }
                            ?>                            
