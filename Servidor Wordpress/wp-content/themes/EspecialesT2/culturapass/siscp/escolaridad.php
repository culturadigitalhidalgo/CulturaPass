...
//plantilla V4.0
    ...
    $tablabase="escolaridad";
    $idbase="idescolaridad";
    $paginabase=$selfpag;
    
    //Variables Globales del Archivo (pueden ser omitidas) texto desplegado en los formularios de catálogos
    $textoencabezado="Escolaridad";
    $textoencabezadoforms="Escolaridad";
    
    //Variables para obtención de información en grid data table
    $camposrs="idescolaridad as IDEscolaridad, escolaridad as Escolaridad, (case when status=1 then 'Activo' else 'Baja' end ) as Status";
    $tablars=$tablabase;
    $wherers="1";
    $idaccion="IDEscolaridad";
    
    //Variables para generación de formularios y almacen de datos $campos['Nombre_campo']="aray de atributos";
    $atribs['tipo']="texto";
    $atribs['label']="IDEscolaridad";
    $atribs['obligatorio']=true;                
    $campos['idescolaridad']=$atribs;
    unset($atribs);
    $atribs['tipo']="texto";
    $atribs['label']="Escolaridad";
    $atribs['obligatorio']=true; 
    $campos['escolaridad']=$atribs;
    unset($atribs);
    $atribs['tipo']="select2";
    $atribs['label']="Status";
    $atribs['obligatorio']=true;                
    $atribs['camposselect']="1^Activo¬0^Baja";            
    $atribs['defaultselect']='1';                
    $campos['status']=$atribs;
    unset($atribs);
                            /***********************Configuración****************************/
                    ...
                    $.confirm({
                        ...
                        onContentReady: function () {
                            ...
                            $('#FormModifica input, #FormModifica select').each(function(){
                                $(this).val(variables[$(this).attr('id').slice(0,-1)]); 
                            });
                        },
                        buttons: {
                            formSubmit: {
                                ...
                                while ($row = mysql_fetch_array($data)){
                                    echo '<tr>';
                                        ...
                                        echo '<td style="width:50px; min-width:50px; text-align:right;">
                                            <a id-accion="'.$row[$idaccion].'" class="modificar"><span class="fa fa-pencil-square-o" title="Modificar"></span></a> 
