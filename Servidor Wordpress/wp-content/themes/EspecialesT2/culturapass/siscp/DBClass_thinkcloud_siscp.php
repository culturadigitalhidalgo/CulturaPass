...
//    require_once('singleton/Database.singleton.php');
    require_once('singleton/db_config_thinkcloud_siscp.php');
    require_once('singleton/Database.singleton_thinkcloud_siscp.php');
    
    
//    include( ABSPATH .'wp-includes/class-phpass.php' );
    
    class DBClass_thinkcloud_SISCP{
        private $link="";
//        private $linkloc="";
        
        public function __construct(){
            $db = DatabaseTCSISCP::obtain(DB_SERVER_TC_SISCP, DB_USER_TC_SISCP, DB_PASS_TC_SISCP, DB_DATABASE_TC_SISCP);
            $db->connect();
            $db->query("SET NAMES 'utf8'");   
            
            $this->link=$db; 
            
//            $dbloc = Database::obtain(DB_SERVER, DB_USER, DB_PASS, DB_DATABASE);
//            $dbloc->connect();
//            $dbloc->query("SET NAMES 'utf8'");     
//            
//            $this->linkloc=$dbloc;            
        }  
...
//                $_SESSION['apmaternousuariossh'] = $record['apmaterno'];
        ...
        public function verificar_WPUsr($usuario,$claveplana){
            ...
            $passq=$this->to_query_loc("select * from wp_users where user_login='" . $user_name . "' LIMIT 1");
...
//            $_SESSION['apmaternousuariossh']="";
        ...
        //Query  param: sentencia SQL Local//
//        public function to_query_loc($sql){
//            if($primary_id = $this->linkloc->query($sql)){
//                return $primary_id;
//            }else{
//                return 'Error al Ejecutar la Consulta';
//            }
//        }
        ...
        //************Funcionaes Publicas**********////
        public function save_abono($idculturapass,$cantidad,$idusuariosc,$forma_pago,$idusuariocp,$nombreusrwp=""){
           ...
           $movimientodata['idusuariocp']= $idusuariocp;
           $movimientodata['fecha_hora']=$fecha_full;
           $movimientodata['monto']=$cantidad;
           $movimientodata['idforma_pago']=$forma_pago;
           ...
           $forma_pagostr=$this->obtener_por_id("formas_pago", "idforma_pago", $forma_pago, 'forma_pago');
           ...
           //Validador de la transacción
            ...
            if($valid==true){
                ...
                return "commit ".
                        '<input type="button" style="width:100px; height:30px; padding: 4px;" class="ink-button green" value="Imprimir" onClick="showAndroidToast(\'Imprimiendo...\');PrintTicket(\' \n\n\r '
                        . '  Secretaría de Cultura del \n\r       Estado de Hidalgo \n\r          Cultura Pass \n\n\n\r Comprobante de Abono de Saldo \n\n\n\r'
                        . 'Propietario: '.$nombreusrwp.' \n\rCantidad: $'. number_format($cantidad, 2).' \n\rForma Pago: '.$forma_pagostr.' \n\rFecha/Hora: '.$fecha_full.' \\r\n\n\n\\n\n\')" />';     
            }else{
        ...
        public function save_cargo($idculturapass,$cantidad,$idservicio,$idevento,$fecha,$hora,$idusuariosc,$cantidadb,$estadistica,$idusuariocp,$nombreusrwp="",$nombreevento="",$puntos=0,$movimientopuntos=0){
           $fecha_full=date('Y-m-d H:i:s');
           $idmovimiento= $this->genera_id_movimiento($idculturapass);
           ...
           //Validador de la transacción
            ...
            foreach ($estadisticasb as $key => $value) {
                ...
                $movboletodata['utilizados']=$estadisticab[3];
            ...
            $mensajepuntos="";
            if($puntos>0){
                $puntosdata['idmovimiento']=$idmovimiento;
                $puntosdata['idtipomovimiento']=2;
                $puntosdata['idtipomovimientopuntos']=$movimientopuntos;
                $puntosdata['idculturapass']=$idculturapass;
                $puntosdata['idusuariocp']= $idusuariocp;
                $puntosdata['fecha_hora']=$fecha_full;
                $puntosdata['monto']=$puntos;
                $puntosdata['idusuariosc']=$idusuariosc;
                $puntosdata['iddispositivo']="1";
                $validador= $this->genera_hash_puntos($puntosdata);
                $puntosdata['validador']=$validador;
                
                if(($insertpuntos = $this->to_insert_trans("puntos", $puntosdata))===false){
                    $valid=false;
                    echo 'Error al procesar la información de Puntos<br>';
                }
                
                if($movimientopuntos==1){
                    $mensajepuntos=' \n\rPuntos Acumulados: '. $puntos;
                }else{
                    $mensajepuntos=' \n\rPuntos Remitidos: '. $puntos;
                }
            }
            ...
            if($valid==true){
                ...
                return "commit".
                        '<input type="button" style="width:100px; height:30px; padding: 4px;" class="ink-button green" value="Imprimir" onClick="showAndroidToast(\'Imprimiendo...\');PrintTicket(\' \n\n\r '
                        . '  Secretaría de Cultura del \n\r       Estado de Hidalgo \n\r          Cultura Pass \n\n\n\r        Compra de Boletos \n\n\n\r'
                        . 'Propietario: '.$nombreusrwp.' \n\rCantidad: $'. number_format($cantidad, 2).' '.$mensajepuntos.' \n\rFecha/Hora: '.$fecha_full.' '
                        . '\n\n\n\r      Detalles de Evento '
                        . '\n\rNombre: '.$nombreevento
                        . '\n\rFecha/Hora: '.$fecha.' '.$hora
                        . '\n\rNúmero de Boletos: '.$cantidadb
                        . ' \\r\n\n\n\\n\n\')" />';     
            }else{
                $this->to_query("rollback;");  
                return "rollback"; 
        ...
        public function consulta_saldo($idusuariocp,$nombreusr=""){
//            $idusuariocp = $this->get_usercp($idculturapass);
            ...
            if($validmovs){
                ...
                $abonos=0;
                $cargos=0;
                $infocp= $this->to_query("SELECT sum(monto) as abonos from puntos where idusuariocp='".$idusuariocp."' and idtipomovimientopuntos=1");
                while($infocprow=mysqli_fetch_array($infocp)){
                    $abonos= $infocprow['abonos'];
                }
                
                $infocp= $this->to_query("SELECT sum(monto) as cargos from puntos where idusuariocp='".$idusuariocp."' and idtipomovimientopuntos=2");
                while($infocprow=mysqli_fetch_array($infocp)){
                    $cargos= $infocprow['cargos'];
                }
                ...
                echo'<span id="usuariocp_calc1">Propietario: <b>'.$nombreusr.'</b></span><br>'
                        . '<span id="saldocp_calc1" saldo="'.$saldo.'" saldoformat="'.number_format($saldo, 2).'">Saldo en la Tarjeta: <b>$'. number_format($saldo, 2)."</b></span><br>"
                        . '<span id="saldocp_calc2" saldo="'.$saldo.'">Saldo en Puntos: <b>'.$puntos." </b></span><br>"
                        . "<br>"
                        . '<input type="button" style="width:100px; height:30px; padding: 4px;" class="ink-button green" value="Imprimir" onClick="showAndroidToast(\'Imprimiendo...\');PrintTicket(\' \n\n\r   Secretaría de Cultura del \n\r       Estado de Hidalgo \n\r          Cultura Pass \n\n\n\rPropietario: '.$nombreusr.' \n\rSaldo en la Tarjeta: $'. number_format($saldo, 2).' \n\rSaldo en Puntos: '. $puntos.' \n\rFecha/Hora: '.$fecha_full.' \\r\n\n\n\\n\n\')" />'
                        . ' <input type="button" style="width:150px; height:30px; padding: 4px;" class="ink-button green" value="Ver Movimientos" id="showmovs" />';
        ...
        public function consulta_movimientos($idusuariocp,$nombreusr="",$nmovs){
//            $idusuariocp = $this->get_usercp($idculturapass);           
            $movimientodom="";
            $validmovs=$this->validar_movimientos($idusuariocp);
            
            if($validmovs){                
                $infocp= $this->to_query("SELECT fecha_hora,monto,(case when idtipomovimiento=1 then concat('Abono (',(select forma_pago from formas_pago fp where fp.idforma_pago = movimientos.idforma_pago),')') else 'Cargo' end ) as tmovimiento FROM `movimientos` WHERE `idusuariocp`=$idusuariocp ORDER by `fecha_hora` desc limit $nmovs");
                while($infocprow=mysqli_fetch_array($infocp)){
                    if($movimientodom===""){
                        $movimientodom='<tr><td style="max-width:120px;">'.$infocprow['fecha_hora'].'</td><td style="text-align:center; min-width:90px;">$'.number_format($infocprow['monto'], 2).'</td><td style="text-align:center;">'.$infocprow['tmovimiento'].'</td></td>';
                    }else{
                        $movimientodom.='<tr><td style="max-width:120px;">'.$infocprow['fecha_hora'].'</td><td style="text-align:center; min-width:90px;">$'.number_format($infocprow['monto'], 2).'</td><td style="text-align:center;">'.$infocprow['tmovimiento'].'</td></td>';
                    }
                }
...
//                echo'<span id="usuariocp_calc1">Propietario: <b>'.$nombreusr.'</b></span><br>'
//                        . '<span id="saldocp_calc1" saldo="'.$saldo.'" saldoformat="'.number_format($saldo, 2).'">Saldo en la Tarjeta: <b>$'. number_format($saldo, 2)."</b></span>"
//                        . "<br>"
//                        . '<input type="button" style="width:100px; height:30px; padding: 8px;" class="ink-button green" value="Imprimir" onClick="showAndroidToast(\'Imprimiendo...\');PrintTicket(\' \n\n\r Secretaría de Cultura Hidalgo \n\r         Cultura Pass \n\n\n\r Propietario: '.$nombreusr.'\ \n\r Saldo en la Tarjeta: $'. number_format($saldo, 2).' \n\r Fecha/Hora: '.$fecha_full.' \\r\n\n\n\\n\n\')" />'
//                        . ' <input type="button" style="width:150px; height:30px; padding: 8px;" class="ink-button green" value="Ver Movimientos" id="showmovs" />';
               echo $movimientodom;
        ...
        public function consulta_saldo_noprint($idusuariocp,$nombreusr=""){
//            $idusuariocp = $this->get_usercp($idculturapass);
            $puntos=0;
            $saldo=0;
            ...
            if($validmovs){
                ...
                echo'<span id="usuariocp_calc1">Propietario: <br><b>'.$nombreusr.'</b></span><br>'
                        . '<span id="saldocp_calc1" saldo="'.$saldo.'" saldoformat="'.number_format($saldo, 2).'">Saldo en la Tarjeta: <br><b>$'. number_format($saldo, 2)."</b></span>"
                        . "<br>";
                ...
                $puntos=$abonos-$cargos;
                echo'<span id="saldocp_calc2" saldo="'.$saldo.'">Saldo en Puntos: <b>'.$puntos." </b></span><br>";
            }else{
                echo '<b style="color:red;" class="push-center"><i class="fa fa-warning"></i> Movimientos Corruptos!<br>La tarjeta se desactivará y se comenzará investigación.</b>';
            }
        ...
        public function validar_cp_movs($idusuariocp){
//            $idusuariocp = $this->get_usercp($idculturapass);
        ...
        public function get_boletospreventa($idculturapass,$idusuariowp){
            return $boletos=$this->get_boletos_preventa($idusuariowp);
        }
        
        public function acces_preventa($idculturapass, $estadistica){
            echo $updateestdis=$this->update_boletosprev($estadistica);
        }
        ...
        private function get_usercp($idculturapass){
            ...
            $wpu= $this->to_query_loc("SELECT user_id FROM wp_usermeta WHERE meta_key='cp_id_culturapass' and meta_value='".$idculturapass."'");
        ...
        private function update_boletosprev($boletos){
            $this->to_query("begin;");
            $valid=true;
            $boletosarray= explode("|", $boletos);
            
            foreach ($boletosarray as $boletosestadis) {
                $bolestosestadisarray= explode(";", $boletosestadis);
                
                //Guardado de información en la tabla declarada en la variable global $tablabase y la información en forma de Array $data
                if(($update = $this->to_query("update boletos set utilizados = (utilizados+".$bolestosestadisarray[7].") where idmovimiento='".$bolestosestadisarray[3]."' and"
                        . " idtipomovimiento='".$bolestosestadisarray[4]."' and idservicio='1' and idevento='".$bolestosestadisarray[0]."' and idgrupoedad='".$bolestosestadisarray[5]."' "
                        . " and sexo='".$bolestosestadisarray[6]."' and fecha='".$bolestosestadisarray[1]."' and hora='".$bolestosestadisarray[2]."' "))==0){
                    $valid=false;
                    echo 'Error al procesar la información<br>';
                }
        ...
        private function get_boletos_preventa($idusWP){
            $idusuario=0;
            
            $boletos= $this->to_query("SELECT boletos.idevento,boletos.fecha,boletos.hora,boletos.cantidad,boletos.utilizados, idmovimiento as key1, idtipomovimiento as key2, idgrupoedad as key3, sexo as key4 
                FROM boletos join descmovimientos using(idmovimiento,idtipomovimiento,idservicio) join movimientos using(idmovimiento,idtipomovimiento) 
                WHERE idusuariocp='".$idusWP."' and boletos.cantidad>utilizados order by idevento,fecha,hora,key3,key4");
        ...
        private function genera_hash_puntos($movimientodata){
            $hash="";
            ...
            $thirdkey= md5($movimientodata['idmovimiento']);
            $fourthkey= md5($movimientodata['idculturapass']+$movimientodata['idusuariocp']);
            $fifthkey= md5($movimientodata['idtipomovimientopuntos']);
            
            $hash= md5($firstkey+$secondkey+$thirdkey+$fourthkey+$fifthkey);
            $hash='$CPM'.$hash;
        ...
        private function validar_movimientos($idusuario) {
            ...
            $movimientospuntosq=$this->to_query("select * from puntos where idusuariocp='$idusuario'");
            while($movimientospr=mysqli_fetch_array($movimientospuntosq)){
                $puntosdata['idmovimiento']=$movimientospr['idmovimiento'];
                $puntosdata['idtipomovimiento']=$movimientospr['idtipomovimiento'];
                $puntosdata['idtipomovimientopuntos']=$movimientospr['idtipomovimientopuntos'];
                $puntosdata['idculturapass']=$movimientospr['idculturapass'];
                $puntosdata['idusuariocp']= $movimientospr['idusuariocp'];
                $puntosdata['fecha_hora']=$movimientospr['fecha_hora'];
                $puntosdata['monto']=$movimientospr['monto'];
                $puntosdata['idusuariosc']=$movimientospr['idusuariosc'];
                $puntosdata['iddispositivo']=$movimientospr['iddispositivo'];
                $validador= $this->genera_hash_puntos($puntosdata);
                $validadorbd=$movimientospr['validador'];
                if($validador!==$validadorbd){
                    $invalida['validmov']=0;
                    $this->to_update_trans("movimientos", $invalida, "idmovimiento='".$puntosdata['idmovimiento']."' and idtipomovimiento='".$puntosdata['idtipomovimiento']."'");                    
                    $result =false;
                }
            }
