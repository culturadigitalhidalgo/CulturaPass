...
//            $_SESSION['apmaternousuariossh']="";
        ...
        public function obtener_plantilla($tipo,$id,$atribs,$update){
            ...
            switch ($tipo){
                ...
                case 'divisor3':
                    ...
                    break;
                case 'div':
                    $plantilla.= '<div class=" control-group gutters all-100" id="div'.$id.'">     
                        <label id="label'.$id.'" for="'.$id.'" class="all-%_ancholabel_%">'.$atribs['label'].'</label>';
                        $plantilla.= '<div class="all-%_anchocampo_%">';
                            $plantilla.= '<div id="'.$id.'" >';
                                if(isset($atribs['content'])){
                                    $plantilla.= $atribs['content'];
                                }
                            $plantilla.= '</div>';
                        $plantilla.= '</div>';
                    $plantilla.= '</div>';                       
                ...
                case 'texto':
                    ...
                    $plantilla.= '" type="text" id="'.$id;
                        if($update){$plantilla.="m";}
                    $plantilla.= '" ';
                    if(isset($atribs['attr'])){$plantilla.= '  '.$atribs['attr'].' ';}
                ...
                case 'number':
                    ...
                    $plantilla.= '" id="div'.$id.'">     
                        ...
                        if($update){$plantilla.="m";}
                    $plantilla.= '" ';
                        if(isset($atribs['attrlabel'])){
                            $plantilla.= $atribs['attrlabel'];
                        }
                    $plantilla.= ' class="all-%_ancholabel_%">'.$atribs['label'].'</label>
                        <div class="all-%_anchocampo_%">
                            <input  class="all-';
                    ...
                    //Parche para mostrar % en el form de cobro de CP
                    if($id==="descuentoevent"){
                        $plantilla.='<span class="all-10" style="text-align: center;"><b>%</b></span>';
                    }
                    $plantilla.='
                        </div>';                       
                    break;
                case 'hidden':
                    $plantilla.= ' <input type="hidden" id="'.$id.'" value="'.$atribs['value'].'" />';                       
                    break;
            }
        ...
        public function get_usercp_class($idculturapass){
            return $idusuariocp = $this->get_usercp($idculturapass);
        }
        
        public function get_username_class($idusuario){
            return $nombreusuariocp = $this->get_usernamecp($idusuario);
        }
        
        public function get_eventname_class($idevento){
            return $nombreevento = $this->get_eventname($idevento);
        }
        
        public function get_eventplace_class($idevento){
            return $lugarevento = $this->get_eventplace($idevento);
        }
        //************Funcionaes Privadas**********////
        ...
        private function get_eventname($idevento){
            $nombreevento="";
            
            $wpu= $this->to_query("SELECT post_title FROM wp_posts WHERE ID='".$idevento."'");
            while($wpur=mysqli_fetch_array($wpu)){
                $nombreevento=$wpur['post_title'];
            }
            
            return $nombreevento;            
        }
        
        private function get_eventplace($idevento){
            $lugarevento="";
            
            $wpu= $this->to_query("(select wpp.post_title from wp_posts wpp where wpp.ID =concat('',(select meta_value from wp_postmeta a where a.meta_key='lugar' and a.post_id='".$idevento."' limit 1)))");
            while($wpur=mysqli_fetch_array($wpu)){
                $lugarevento=$wpur['post_title'];
            }
            
            return $lugarevento;            
        }
        
        private function get_usernamecp($idusuario){
            $nombreusr="";
            
            $wpu= $this->to_query("SELECT meta_value FROM wp_usermeta WHERE meta_key='first_name' and user_id='".$idusuario."'");
            while($wpur=mysqli_fetch_array($wpu)){
                $nombreusr.=$wpur['meta_value'];
            }
            $wpu= $this->to_query("SELECT meta_value FROM wp_usermeta WHERE meta_key='last_name' and user_id='".$idusuario."'");
            while($wpur=mysqli_fetch_array($wpu)){
                $nombreusr.=' '.$wpur['meta_value'];
            }
